...
void AMySQLDBConnectionActor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	for(auto& Task : UpdateQueryTasks)
	{
		if(Task && !Task->IsDone())
		{
	...
	for(auto& Task : SelectQueryTasks)
	{
		if(Task && !Task->IsDone())
		{
			Task->EnsureCompletion();
...
int32 AMySQLDBConnectionActor::GenerateQueryID(int32 ConnectionID)
{
	...
	int32 QueryID = ConnectionToNextQueryIDMap[ConnectionID];
...
{
	int32 ConnectionID = SQLConnectors.Num() - 1;
	if(ConnectionID >= 0)
	{
